...
pub trait TargetedPickup: RumblePickupComponent {
	fn get_b_can_target_ball(&self) -> bool {
		unsafe { SpecialPickup_Targeted_TA_Get_bCanTargetBall(self.addr()) }
	}
	fn get_b_can_target_cars(&self) -> bool {
		unsafe { SpecialPickup_Targeted_TA_Get_bCanTargetCars(self.addr()) }
	}
	fn get_b_can_target_enemy_cars(&self) -> bool {
		unsafe { SpecialPickup_Targeted_TA_Get_bCanTargetEnemyCars(self.addr()) }
	}
	fn get_b_can_target_team_cars(&self) -> bool {
		unsafe { SpecialPickup_Targeted_TA_Get_bCanTargetTeamCars(self.addr()) }
	}
	fn get_b_use_directional_targeting(&self) -> bool {
		unsafe { SpecialPickup_Targeted_TA_Get_bUseDirectionalTargeting(self.addr()) }
	}
	fn get_b_require_trace(&self) -> bool {
		unsafe { SpecialPickup_Targeted_TA_Get_bRequireTrace(self.addr()) }
	}
	fn get_range(&self) -> f32 {
		unsafe { SpecialPickup_Targeted_TA_Get_Range(self.addr()) }
	}
	fn get_directional_targeting_accuracy(&self) -> f32 {
		unsafe { SpecialPickup_Targeted_TA_Get_DirectionalTargetingAccuracy(self.addr()) }
	}
	fn get_client_target(&self) -> Option<RBActorWrapper> {
		unsafe { RBActorWrapper::try_new(SpecialPickup_Targeted_TA_Get_ClientTarget(self.addr())) }
	}
	fn get_targeted(&self) -> Option<RBActorWrapper> {
		unsafe { RBActorWrapper::try_new(SpecialPickup_Targeted_TA_Get_Targeted(self.addr())) }
	}
	fn get_client_target2(&self) -> Option<RBActorWrapper> {
		unsafe { RBActorWrapper::try_new(SpecialPickup_Targeted_TA_GetClientTarget2(self.addr())) }
	}
	...
	fn try_activate(&self, target_override: RBActorWrapper) -> bool {
		unsafe { SpecialPickup_Targeted_TA_TryActivate(self.addr(), target_override.addr()) }
	}
	fn validate_target_trace(&self, in_target: RBActorWrapper) -> bool {
		unsafe { SpecialPickup_Targeted_TA_ValidateTargetTrace(self.addr(), in_target.addr()) }
	}
	fn validate_target(&self, in_target: RBActorWrapper) -> bool {
		unsafe { SpecialPickup_Targeted_TA_ValidateTarget(self.addr(), in_target.addr()) }
	}
	fn get_target(&self) -> Option<RBActorWrapper> {
		unsafe { RBActorWrapper::try_new(SpecialPickup_Targeted_TA_GetTarget(self.addr())) }
	}
}
