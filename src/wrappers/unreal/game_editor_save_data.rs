...
pub trait GameEditorSaveData: SaveData {
	...
	fn get_training_data(&self) -> Option<TrainingEditorSaveDataWrapper> {
		unsafe { TrainingEditorSaveDataWrapper::try_new(SaveData_GameEditor_Training_TA_Get_TrainingData(self.addr())) }
	}
	fn get_player_team_number(&self) -> i32 {
		unsafe { SaveData_GameEditor_Training_TA_Get_PlayerTeamNumber(self.addr()) }
	}
	fn get_b_unowned(&self) -> bool {
		unsafe { SaveData_GameEditor_Training_TA_Get_bUnowned(self.addr()) }
	}
	fn get_shots_completed(&self) -> i32 {
		unsafe { SaveData_GameEditor_Training_TA_Get_ShotsCompleted(self.addr()) }
	}
	...
	fn get_training_save_type(&self, b_owned: bool, b_favorited: bool) -> u8 {
		unsafe { SaveData_GameEditor_Training_TA_GetTrainingSaveType(self.addr(), b_owned, b_favorited) }
	}
	fn init(&self) {
		unsafe {
