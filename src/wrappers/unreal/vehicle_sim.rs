...
pub trait VehicleSim: Object {
	...
	fn get_drive_torque(&self) -> f32 {
		unsafe { VehicleSim_TA_Get_DriveTorque(self.addr()) }
	}
	fn get_brake_torque(&self) -> f32 {
		unsafe { VehicleSim_TA_Get_BrakeTorque(self.addr()) }
	}
	fn get_stop_threshold(&self) -> f32 {
		unsafe { VehicleSim_TA_Get_StopThreshold(self.addr()) }
	}
	fn get_idle_brake_factor(&self) -> f32 {
		unsafe { VehicleSim_TA_Get_IdleBrakeFactor(self.addr()) }
	}
	fn get_opposite_brake_factor(&self) -> f32 {
		unsafe { VehicleSim_TA_Get_OppositeBrakeFactor(self.addr()) }
	}
	fn get_b_use_ackermann_steering(&self) -> bool {
		unsafe { VehicleSim_TA_Get_bUseAckermannSteering(self.addr()) }
	}
	fn get_b_was_attached(&self) -> bool {
		unsafe { VehicleSim_TA_Get_bWasAttached(self.addr()) }
	}
	fn get_output_throttle(&self) -> f32 {
		unsafe { VehicleSim_TA_Get_OutputThrottle(self.addr()) }
	}
	fn get_output_steer(&self) -> f32 {
		unsafe { VehicleSim_TA_Get_OutputSteer(self.addr()) }
	}
	fn get_output_brake(&self) -> f32 {
		unsafe { VehicleSim_TA_Get_OutputBrake(self.addr()) }
	}
	fn get_output_handbrake(&self) -> f32 {
		unsafe { VehicleSim_TA_Get_OutputHandbrake(self.addr()) }
	}
	fn get_vehicle(&self) -> Option<VehicleWrapper> {
		unsafe { VehicleWrapper::try_new(VehicleSim_TA_Get_Vehicle(self.addr())) }
	}
	fn get_car(&self) -> Option<CarWrapper> {
		unsafe { CarWrapper::try_new(VehicleSim_TA_Get_Car(self.addr())) }
	}
	fn get_steering_sensitivity(&self) -> f32 {
		unsafe { VehicleSim_TA_Get_SteeringSensitivity(self.addr()) }
	}
}
