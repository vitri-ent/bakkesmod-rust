...
pub trait StatGraphSystem: Object {
	fn get_graph_sample_time(&self) -> f32 {
		unsafe { StatGraphSystem_TA_Get_GraphSampleTime(self.addr()) }
	}
	fn get_graph_level(&self) -> u8 {
		unsafe { StatGraphSystem_TA_Get_GraphLevel(self.addr()) }
	}
	fn get_perf_stat_graph(&self) -> Option<PerfStatGraphWrapper> {
		unsafe { PerfStatGraphWrapper::try_new(StatGraphSystem_TA_Get_PerfStatGraph(self.addr())) }
	}
	fn get_net_stat_graph(&self) -> Option<NetStatGraphWrapper> {
		unsafe { NetStatGraphWrapper::try_new(StatGraphSystem_TA_Get_NetStatGraph(self.addr())) }
	}
	fn get_input_buffer_graph(&self) -> Option<InputBufferGraphWrapper> {
		unsafe { InputBufferGraphWrapper::try_new(StatGraphSystem_TA_Get_InputBufferGraph(self.addr())) }
	}
	...
	fn get_graph_sample_time2(&self, level: u8) -> f32 {
		unsafe { StatGraphSystem_TA_GetGraphSampleTime2(self.addr(), level) }
	}
