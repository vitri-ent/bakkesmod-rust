...
pub trait StatGraph: Object {
	fn get_record_settings(&self) -> Option<SampleRecordSettingsWrapper> {
		unsafe { SampleRecordSettingsWrapper::try_new(StatGraph_TA_Get_RecordSettings(self.addr())) }
	}
	...
	fn add_sample_history(&self, history: SampleHistoryWrapper) -> Option<SampleHistoryWrapper> {
		unsafe { SampleHistoryWrapper::try_new(StatGraph_TA_AddSampleHistory(self.addr(), history.addr())) }
	}
}
