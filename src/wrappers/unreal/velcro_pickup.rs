...
pub trait VelcroPickup: RumblePickupComponent {
	...
	fn get_b_use_real_offset(&self) -> bool {
		unsafe { SpecialPickup_BallVelcro_TA_Get_bUseRealOffset(self.addr()) }
	}
	fn get_b_hit(&self) -> bool {
		unsafe { SpecialPickup_BallVelcro_TA_Get_bHit(self.addr()) }
	}
	fn get_b_broken(&self) -> bool {
		unsafe { SpecialPickup_BallVelcro_TA_Get_bBroken(self.addr()) }
	}
	fn get_after_hit_duration(&self) -> f32 {
		unsafe { SpecialPickup_BallVelcro_TA_Get_AfterHitDuration(self.addr()) }
	}
	fn get_post_break_duration(&self) -> f32 {
		unsafe { SpecialPickup_BallVelcro_TA_Get_PostBreakDuration(self.addr()) }
	}
	fn get_min_break_force(&self) -> f32 {
		unsafe { SpecialPickup_BallVelcro_TA_Get_MinBreakForce(self.addr()) }
	}
	fn get_min_break_time(&self) -> f32 {
		unsafe { SpecialPickup_BallVelcro_TA_Get_MinBreakTime(self.addr()) }
	}
	fn get_check_last_touch_rate(&self) -> f32 {
		unsafe { SpecialPickup_BallVelcro_TA_Get_CheckLastTouchRate(self.addr()) }
	}
	fn get_welded_ball(&self) -> Option<BallWrapper> {
		unsafe { BallWrapper::try_new(SpecialPickup_BallVelcro_TA_Get_WeldedBall(self.addr())) }
	}
	fn get_old_ball_mass(&self) -> f32 {
		unsafe { SpecialPickup_BallVelcro_TA_Get_OldBallMass(self.addr()) }
	}
	fn get_attach_time(&self) -> f32 {
		unsafe { SpecialPickup_BallVelcro_TA_Get_AttachTime(self.addr()) }
	}
	fn get_last_touch_check_time(&self) -> f32 {
		unsafe { SpecialPickup_BallVelcro_TA_Get_LastTouchCheckTime(self.addr()) }
	}
	fn get_break_time(&self) -> f32 {
		unsafe { SpecialPickup_BallVelcro_TA_Get_BreakTime(self.addr()) }
	}
	fn do_break(&self) {
		unsafe {
