...
pub trait DemolishPickup: RumblePickupComponent {
	fn get_demolish_target(&self) -> u8 {
		unsafe { SpecialPickup_Demolish_TA_Get_DemolishTarget(self.addr()) }
	}
	fn get_demolish_speed(&self) -> u8 {
		unsafe { SpecialPickup_Demolish_TA_Get_DemolishSpeed(self.addr()) }
	}
	fn get_old_target(&self) -> u8 {
		unsafe { SpecialPickup_Demolish_TA_Get_OldTarget(self.addr()) }
	}
	fn get_old_speed(&self) -> u8 {
		unsafe { SpecialPickup_Demolish_TA_Get_OldSpeed(self.addr()) }
	}
	fn pickup_end(&self) {
		unsafe {
