...
pub trait Ball: RBActor {
	...
	fn predict_position(&self, time_ahead: f32) -> PredictionInfo {
		...
		unsafe {
			Ball_PredictPosition(self.addr(), time_ahead, result_ptr);
		}
		result
	}

	fn get_end_of_game_fx_archetype(&self) -> Option<FXActorWrapper> {
		unsafe { FXActorWrapper::try_new(Ball_TA_Get_EndOfGameFXArchetype(self.addr())) }
	}
	fn get_b_allow_player_explosion_override(&self) -> bool {
		unsafe { Ball_TA_Get_bAllowPlayerExplosionOverride(self.addr()) }
	}
	fn get_b_notify_ground_hit(&self) -> bool {
		unsafe { Ball_TA_Get_bNotifyGroundHit(self.addr()) }
	}
	fn get_b_end_of_game_hidden(&self) -> bool {
		unsafe { Ball_TA_Get_bEndOfGameHidden(self.addr()) }
	}
	fn get_b_fade_in(&self) -> bool {
		unsafe { Ball_TA_Get_bFadeIn(self.addr()) }
	}
	fn get_b_fade_out(&self) -> bool {
		unsafe { Ball_TA_Get_bFadeOut(self.addr()) }
	}
	fn get_b_prediction_on_ground(&self) -> bool {
		unsafe { Ball_TA_Get_bPredictionOnGround(self.addr()) }
	}
	fn get_b_can_be_attached(&self) -> bool {
		unsafe { Ball_TA_Get_bCanBeAttached(self.addr()) }
	}
	fn get_b_item_freeze(&self) -> bool {
		unsafe { Ball_TA_Get_bItemFreeze(self.addr()) }
	}
	...
	fn get_radius(&self) -> f32 {
		unsafe { Ball_TA_Get_Radius(self.addr()) }
	}
	fn get_visual_radius(&self) -> f32 {
		unsafe { Ball_TA_Get_VisualRadius(self.addr()) }
	}
	fn get_last_calculate_car_hit(&self) -> f32 {
		unsafe { Ball_TA_Get_LastCalculateCarHit(self.addr()) }
	}
	...
	fn get_last_hit_world_time(&self) -> f32 {
		unsafe { Ball_TA_Get_LastHitWorldTime(self.addr()) }
	}
	fn get_replicated_ball_scale(&self) -> f32 {
		unsafe { Ball_TA_Get_ReplicatedBallScale(self.addr()) }
	}
	fn get_replicated_world_bounce_scale(&self) -> f32 {
		unsafe { Ball_TA_Get_ReplicatedWorldBounceScale(self.addr()) }
	}
	fn get_replicated_ball_gravity_scale(&self) -> f32 {
		unsafe { Ball_TA_Get_ReplicatedBallGravityScale(self.addr()) }
	}
	fn get_replicated_ball_max_linear_speed_scale(&self) -> f32 {
		unsafe { Ball_TA_Get_ReplicatedBallMaxLinearSpeedScale(self.addr()) }
	}
	fn get_replicated_added_car_bounce_scale(&self) -> f32 {
		unsafe { Ball_TA_Get_ReplicatedAddedCarBounceScale(self.addr()) }
	}
	fn get_additional_car_ground_bounce_scale_z(&self) -> f32 {
		unsafe { Ball_TA_Get_AdditionalCarGroundBounceScaleZ(self.addr()) }
	}
	fn get_additional_car_ground_bounce_scale_xy(&self) -> f32 {
		unsafe { Ball_TA_Get_AdditionalCarGroundBounceScaleXY(self.addr()) }
	}
	fn get_hit_team_num(&self) -> u8 {
		unsafe { Ball_TA_Get_HitTeamNum(self.addr()) }
	}
	fn get_game_event(&self) -> Option<ServerWrapper> {
		unsafe { ServerWrapper::try_new(Ball_TA_Get_GameEvent(self.addr())) }
	}
	fn get_explosion_time(&self) -> f32 {
		unsafe { Ball_TA_Get_ExplosionTime(self.addr()) }
	}
	...
	fn get_prediction_timestep(&self) -> f32 {
		unsafe { Ball_TA_Get_PredictionTimestep(self.addr()) }
	}
	fn get_last_prediction_time(&self) -> f32 {
		unsafe { Ball_TA_Get_LastPredictionTime(self.addr()) }
	}
	fn get_ground_force(&self) -> f32 {
		unsafe { Ball_TA_Get_GroundForce(self.addr()) }
	}
	fn get_current_affector(&self) -> Option<CarWrapper> {
		unsafe { CarWrapper::try_new(Ball_TA_Get_CurrentAffector(self.addr())) }
	}
	...
	fn should_draw_trajectory(&self) -> bool {
		unsafe { Ball_TA_ShouldDrawTrajectory(self.addr()) }
	}
	fn get_additional_car_bounce_scale_z(&self, car: CarWrapper) -> f32 {
		unsafe { Ball_TA_GetAdditionalCarBounceScaleZ(self.addr(), car.addr()) }
	}
	...
	fn is_round_active(&self) -> bool {
		unsafe { Ball_TA_IsRoundActive(self.addr()) }
	}
	fn turn_off(&self) {
		unsafe {
