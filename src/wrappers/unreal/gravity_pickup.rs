...
pub trait GravityPickup: RumblePickupComponent {
	fn get_ball_gravity(&self) -> f32 {
		unsafe { SpecialPickup_BallGravity_TA_Get_BallGravity(self.addr()) }
	}
	fn get_range(&self) -> f32 {
		unsafe { SpecialPickup_BallGravity_TA_Get_Range(self.addr()) }
	}
	...
	fn get_b_deactivate_on_touch(&self) -> bool {
		unsafe { SpecialPickup_BallGravity_TA_Get_bDeactivateOnTouch(self.addr()) }
	}
	fn get_record_ball_hit_rate(&self) -> f32 {
		unsafe { SpecialPickup_BallGravity_TA_Get_RecordBallHitRate(self.addr()) }
	}
	fn get_last_recorded_ball_hit_time(&self) -> f32 {
		unsafe { SpecialPickup_BallGravity_TA_Get_LastRecordedBallHitTime(self.addr()) }
	}
	fn get_prev_ball(&self) -> Option<BallWrapper> {
		unsafe { BallWrapper::try_new(SpecialPickup_BallGravity_TA_Get_PrevBall(self.addr())) }
	}
