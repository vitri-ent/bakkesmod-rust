...
pub trait GameEditor: Server {
	fn get_active_round_number(&self) -> i32 {
		unsafe { GameEvent_GameEditor_TA_Get_ActiveRoundNumber(self.addr()) }
	}
	fn get_max_rounds(&self) -> i32 {
		unsafe { GameEvent_GameEditor_TA_Get_MaxRounds(self.addr()) }
	}
	fn get_history_position(&self) -> i32 {
		unsafe { GameEvent_GameEditor_TA_Get_HistoryPosition(self.addr()) }
	}
	fn get_max_undo_history(&self) -> i32 {
		unsafe { GameEvent_GameEditor_TA_Get_MaxUndoHistory(self.addr()) }
	}
	fn get_fx_actor_archetype(&self) -> Option<FXActorWrapper> {
		unsafe { FXActorWrapper::try_new(GameEvent_GameEditor_TA_Get_FXActorArchetype(self.addr())) }
	}
	fn get_fx_actor(&self) -> Option<FXActorWrapper> {
		unsafe { FXActorWrapper::try_new(GameEvent_GameEditor_TA_Get_FXActor(self.addr())) }
	}
	...
	fn can_que_save_replay(&self) -> bool {
		unsafe { GameEvent_GameEditor_TA_CanQueSaveReplay(self.addr()) }
	}
	fn should_update_crosshair(&self) -> bool {
		unsafe { GameEvent_GameEditor_TA_ShouldUpdateCrosshair(self.addr()) }
	}
	fn get_player_team_number(&self) -> i32 {
		unsafe { GameEvent_GameEditor_TA_GetPlayerTeamNumber(self.addr()) }
	}
	fn can_add_history(&self) -> bool {
		unsafe { GameEvent_GameEditor_TA_CanAddHistory(self.addr()) }
	}
	...
	fn get_other_history_type(&self, history_type: u8) -> u8 {
		unsafe { GameEvent_GameEditor_TA_GetOtherHistoryType(self.addr(), history_type) }
	}
	...
	fn destroy_actor(&self, a: ActorWrapper, history_type: u8) -> bool {
		unsafe { GameEvent_GameEditor_TA_DestroyActor(self.addr(), a.addr(), history_type) }
	}
	fn is_in_editor_mode(&self) -> bool {
		unsafe { GameEvent_GameEditor_TA_IsInEditorMode(self.addr()) }
	}
	...
	fn round_contains_a_switch(&self) -> bool {
		unsafe { GameEvent_GameEditor_TA_RoundContainsASwitch(self.addr()) }
	}
	...
	fn can_change_team(&self) -> bool {
		unsafe { GameEvent_GameEditor_TA_CanChangeTeam(self.addr()) }
	}
	...
	fn should_reset_balls(&self) -> bool {
		unsafe { GameEvent_GameEditor_TA_ShouldResetBalls(self.addr()) }
	}
	...
	fn choose_team(&self, team_index: i32, player: PlayerControllerWrapper) -> bool {
		unsafe { GameEvent_GameEditor_TA_ChooseTeam(self.addr(), team_index, player.addr()) }
	}
	fn init_fx(&self) {
		unsafe {
