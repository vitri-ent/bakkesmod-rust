...
pub trait CameraX: BaseCamera {
	...
	fn get_shake_fov_offset(&self) -> f32 {
		unsafe { Camera_X_Get_ShakeFOVOffset(self.addr()) }
	}
	...
	fn get_b_disable_camera_shake(&self) -> bool {
		unsafe { Camera_X_Get_bDisableCameraShake(self.addr()) }
	}
	fn get_b_snap_next_transition(&self) -> bool {
		unsafe { Camera_X_Get_bSnapNextTransition(self.addr()) }
	}
	fn is_transitioning(&self) -> bool {
		unsafe { Camera_X_IsTransitioning(self.addr()) }
	}
