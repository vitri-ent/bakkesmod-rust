...
pub trait GameSettingPlaylist: Object {
	...
	fn get_player_count(&self) -> i32 {
		unsafe { GameSettingPlaylist_X_Get_PlayerCount(self.addr()) }
	}
	fn get_b_standard(&self) -> bool {
		unsafe { GameSettingPlaylist_X_Get_bStandard(self.addr()) }
	}
	fn get_b_ranked(&self) -> bool {
		unsafe { GameSettingPlaylist_X_Get_bRanked(self.addr()) }
	}
	fn get_b_solo(&self) -> bool {
		unsafe { GameSettingPlaylist_X_Get_bSolo(self.addr()) }
	}
	fn get_b_new(&self) -> bool {
		unsafe { GameSettingPlaylist_X_Get_bNew(self.addr()) }
	}
	fn get_b_apply_quit_penalty(&self) -> bool {
		unsafe { GameSettingPlaylist_X_Get_bApplyQuitPenalty(self.addr()) }
	}
	fn get_b_allow_forfeit(&self) -> bool {
		unsafe { GameSettingPlaylist_X_Get_bAllowForfeit(self.addr()) }
	}
	fn get_b_disable_ranked_reconnect(&self) -> bool {
		unsafe { GameSettingPlaylist_X_Get_bDisableRankedReconnect(self.addr()) }
	}
	fn get_b_ignore_assign_teams(&self) -> bool {
		unsafe { GameSettingPlaylist_X_Get_bIgnoreAssignTeams(self.addr()) }
	}
	fn get_b_kick_on_migrate(&self) -> bool {
		unsafe { GameSettingPlaylist_X_Get_bKickOnMigrate(self.addr()) }
	}
	fn get_b_allow_clubs(&self) -> bool {
		unsafe { GameSettingPlaylist_X_Get_bAllowClubs(self.addr()) }
	}
	fn get_b_players_vs_bots(&self) -> bool {
		unsafe { GameSettingPlaylist_X_Get_bPlayersVSBots(self.addr()) }
	}
	fn get_playlist_id(&self) -> i32 {
		unsafe { GameSettingPlaylist_X_Get_PlaylistId(self.addr()) }
	}
	...
	fn is_lan_match(&self) -> bool {
		unsafe { GameSettingPlaylist_X_IsLanMatch(self.addr()) }
	}
	fn is_private_match(&self) -> bool {
		unsafe { GameSettingPlaylist_X_IsPrivateMatch(self.addr()) }
	}
	fn should_update_skills(&self) -> bool {
		unsafe { GameSettingPlaylist_X_ShouldUpdateSkills(self.addr()) }
	}
	fn is_valid_id(&self, in_playlist_id: i32) -> bool {
		unsafe { GameSettingPlaylist_X_IsValidID(self.addr(), in_playlist_id) }
	}
	fn is_valid(&self) -> bool {
		unsafe { GameSettingPlaylist_X_IsValid(self.addr()) }
	}
}
