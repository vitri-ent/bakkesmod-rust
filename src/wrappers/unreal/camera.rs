...
pub trait Camera: CameraX {
	fn get_location(&self) -> Vector {
		...
		unsafe {
			Camera_GetLocation(self.addr(), result_ptr);
		}
		result
	}
	...
	fn set_location(&self, location: Vector) {
		...
		unsafe {
			Camera_SetLocation(self.addr(), location);
		}
	}
	...
	fn get_rotation(&self) -> Rotator {
		...
		unsafe {
			Camera_GetRotation(self.addr(), result_ptr);
		}
		result
	}
	...
	fn set_rotation(&self, rotation: Rotator) {
		...
		unsafe {
			Camera_SetRotation(self.addr(), rotation);
		}
	}
	...
	fn get_camera_settings(&self) -> ProfileCameraSettings {
		...
		unsafe {
			Camera_GetCameraSettings(self.addr(), result_ptr);
		}
		result
	}
	...
	fn set_camera_settings(&self, settings: ProfileCameraSettings) {
		...
		unsafe {
			Camera_SetCameraSettings(self.addr(), settings);
		}
	}
	...
	fn get_pov(&self) -> POV {
		...
		unsafe {
			Camera_GetPOV(self.addr(), result_ptr);
		}
		result
	}
	...
	fn set_pov(&self, pov: POV) {
		...
		unsafe {
			Camera_SetPOV(self.addr(), pov);
		}
	}

	fn set_fov(&self, fov: f32) {
		unsafe {
			Camera_SetFOV_custom(self.addr(), fov);
		}
	}
	...
	fn set_locked_fov(&self, lock: bool) {
		unsafe {
			Camera_SetLockedFOV(self.addr(), lock);
		}
	}
	...
	fn set_camera_state(&self, state_name: &str) {
		...
		unsafe {
			Camera_SetCameraState(self.addr(), c_string);
		}
	}
	...
	fn linterp(&self, start: Vector, end: Vector, elapsed: f32, speed: f32) -> Vector {
		...
		unsafe {
			Camera_linterp(self.addr(), start, end, elapsed, speed, result_ptr);
		}
		result
	}
	...
	fn get_swivel_fast_speed(&self) -> f32 {
		unsafe { Camera_TA_Get_SwivelFastSpeed(self.addr()) }
	}
	fn get_swivel_die_rate(&self) -> f32 {
		unsafe { Camera_TA_Get_SwivelDieRate(self.addr()) }
	}
	...
	fn get_horizontal_splitscreen_height_offset(&self) -> f32 {
		unsafe { Camera_TA_Get_HorizontalSplitscreenHeightOffset(self.addr()) }
	}
	fn get_horizontal_splitscreen_fov_offset(&self) -> f32 {
		unsafe { Camera_TA_Get_HorizontalSplitscreenFOVOffset(self.addr()) }
	}
	fn get_vertical_splitscreen_fov_offset(&self) -> f32 {
		unsafe { Camera_TA_Get_VerticalSplitscreenFOVOffset(self.addr()) }
	}
	fn get_clip_rate(&self) -> f32 {
		unsafe { Camera_TA_Get_ClipRate(self.addr()) }
	}
	...
	fn get_demolisher(&self) -> Option<RBActorWrapper> {
		unsafe { RBActorWrapper::try_new(Camera_TA_Get_Demolisher(self.addr())) }
	}
	fn get_b_demolished(&self) -> bool {
		unsafe { Camera_TA_Get_bDemolished(self.addr()) }
	}
	fn clip_to_field(&self, camera_location_z: f32) -> f32 {
		unsafe { Camera_TA_ClipToField(self.addr(), camera_location_z) }
	}
	...
	fn get_default_fov_offset(&self) -> f32 {
		unsafe { Camera_TA_GetDefaultFOVOffset(self.addr()) }
	}
	fn get_default_view_height_offset(&self) -> f32 {
		unsafe { Camera_TA_GetDefaultViewHeightOffset(self.addr()) }
	}
	fn update_fov(&self) {
		unsafe {
