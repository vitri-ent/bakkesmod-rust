...
//!
//! Also, add an environment variable called `BAKKESMOD_LIB_PATH` containing the path to `pluginsdk.lib` (e.g.
//! `C:\Program Files (x86)\Steam\steamapps\common\rocketleague\Binaries\Win64\bakkesmod\bakkesmodsdk\lib`).
//!
//!
//! ### Create the project
...
//! Make sure you have exactly one function with the `#[plugin_init]` attribute. This function will be called when the
//! plugin is loaded.
//!
//! ### Building
//! Use `cargo build` or `cargo build --release` to build. A `<pluginname>.dll` file is created in `target/debug` or
//! `target/release`. Copy this file to your `bakkesmod/plugins` folder. It can now be loaded in-game with `plugin load
//! <pluginname>`.
//!
//!
//! # Guide
...
//!
//! BakkesMod plugins are loaded in-game using the `plugin load <pluginname>` command in the console, or automatically
//! when listed in the `plugins.cfg` file. When a plugin is loaded, the function with the `plugin_init` attribute is
//! executed. It makes sense to call this function something like `on_load`.
//! After this, plugin code is only executed through callbacks, which can trigger when e.g. a game event happens, or a
//! console command is executed. Typically, the `on_load` function will register these callbacks, and the rest of the
//! plugin's execution happens inside these callbacks.
//!
//!  As an example:
//! ```rust
...
//!
//! The `wrappers::unreal` module contains auto-generated wrappers around game classes, that are written in
//! unrealscript. Since Rust doesn't have struct inheritance, and since BakkesMod provides only methods on these classes
//! anyway, *traits* are used to model functionality on these wrappers.
//!
...
//! (defined in `Car`) on a `CarWrapper` instance. `CarWrapper` also implements the `Vehicle`, `RBActor` and `Actor`
//! traits, so all methods defined in those traits are also callable on a `CarWrapper` instance. This is how the
//! unrealscript inheritance is mimicked.
//!
