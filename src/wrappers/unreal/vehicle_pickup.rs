...
pub trait VehiclePickup: Actor {
	fn get_respawn_delay(&self) -> f32 {
		unsafe { VehiclePickup_TA_Get_RespawnDelay(self.addr()) }
	}
	fn get_fx_actor_archetype(&self) -> Option<FXActorWrapper> {
		unsafe { FXActorWrapper::try_new(VehiclePickup_TA_Get_FXActorArchetype(self.addr())) }
	}
	fn get_fx_actor(&self) -> Option<FXActorWrapper> {
		unsafe { FXActorWrapper::try_new(VehiclePickup_TA_Get_FXActor(self.addr())) }
	}
	fn get_b_net_relevant(&self) -> bool {
		unsafe { VehiclePickup_TA_Get_bNetRelevant(self.addr()) }
	}
	fn get_b_no_pickup(&self) -> bool {
		unsafe { VehiclePickup_TA_Get_bNoPickup(self.addr()) }
	}
	...
	fn is_touching_a_vehicle(&self) -> bool {
		unsafe { VehiclePickup_TA_IsTouchingAVehicle(self.addr()) }
	}
	...
	fn can_pickup(&self, car: CarWrapper) -> bool {
		unsafe { VehiclePickup_TA_CanPickup(self.addr(), car.addr()) }
	}
