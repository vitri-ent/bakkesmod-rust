...
use crate::internal;
use crate::internal::{DrawableCallback, HookCallback, HookWithCallerCallback, HookWithCallerCallbackInternal, TimeoutCallback};
use crate::wrappers::canvas::Canvas;
use crate::wrappers::unreal::{CameraWrapper, CarWrapper, EngineTAWrapper, PlayerControllerWrapper, ServerWrapper};
...
fn hook_event_internal(name: &str, callback: Box<HookCallback>, post: bool) {
	...
	if post {
		unsafe {
			HookEventPost(id, cb_addr, c_name, c_callback);
		}
	} else {
		unsafe {
			HookEvent(id, cb_addr, c_name, c_callback);
		}
	}
}
...
fn hook_event_with_caller_internal(name: &str, callback: Box<HookWithCallerCallbackInternal>, post: bool) {
	...
	if post {
		unsafe {
			HookEventWithCallerPost(id, cb_addr, c_name, c_callback);
		}
	} else {
		unsafe {
			HookEventWithCaller(id, cb_addr, c_name, c_callback);
		}
	}
}
...
pub fn register_drawable(callback: Box<DrawableCallback>) {
	...
	unsafe {
		RegisterDrawable(id, cb_addr, c_callback);
	}
}
...
pub fn set_timeout(callback: Box<TimeoutCallback>, time: f32) {
	...
	unsafe {
		SetTimeout(id, cb_addr, c_callback, time);
	}
}
