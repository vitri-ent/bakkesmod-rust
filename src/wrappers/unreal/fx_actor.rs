...
pub trait FXActor: Actor {
	fn get_b_deactivate_when_owner_destroyed(&self) -> bool {
		unsafe { FXActor_X_Get_bDeactivateWhenOwnerDestroyed(self.addr()) }
	}
	fn get_b_allow_shadow_casting(&self) -> bool {
		unsafe { FXActor_X_Get_bAllowShadowCasting(self.addr()) }
	}
	fn get_b_auto_activate(&self) -> bool {
		unsafe { FXActor_X_Get_bAutoActivate(self.addr()) }
	}
	fn get_b_render_inactive(&self) -> bool {
		unsafe { FXActor_X_Get_bRenderInactive(self.addr()) }
	}
	fn get_b_active(&self) -> bool {
		unsafe { FXActor_X_Get_bActive(self.addr()) }
	}
	fn get_b_had_owner(&self) -> bool {
		unsafe { FXActor_X_Get_bHadOwner(self.addr()) }
	}
	fn get_parent(&self) -> Option<FXActorWrapper> {
		unsafe { FXActorWrapper::try_new(FXActor_X_Get_Parent(self.addr())) }
	}
	fn get_attachment_actor(&self) -> Option<ActorWrapper> {
		unsafe { ActorWrapper::try_new(FXActor_X_Get_AttachmentActor(self.addr())) }
	}
	fn get_destroy_wait_time(&self) -> f32 {
		unsafe { FXActor_X_Get_DestroyWaitTime(self.addr()) }
	}
	fn get_destroy_time(&self) -> f32 {
		unsafe { FXActor_X_Get_DestroyTime(self.addr()) }
	}
	fn get_edit_id(&self) -> i32 {
		unsafe { FXActor_X_Get_EditID(self.addr()) }
	}
	...
	fn is_locally_controlled(&self) -> bool {
		unsafe { FXActor_X_IsLocallyControlled(self.addr()) }
	}
	fn deactivate(&self) {
		unsafe {
