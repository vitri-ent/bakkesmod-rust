...
pub trait BallFreezePickup: TargetedPickup {
	fn get_freeze_break_fx_archetype(&self) -> Option<FXActorWrapper> {
		unsafe { FXActorWrapper::try_new(SpecialPickup_BallFreeze_TA_Get_FreezeBreakFXArchetype(self.addr())) }
	}
	fn get_freeze_fx_archetype(&self) -> Option<FXActorWrapper> {
		unsafe { FXActorWrapper::try_new(SpecialPickup_BallFreeze_TA_Get_FreezeFXArchetype(self.addr())) }
	}
	fn get_b_maintain_momentum(&self) -> bool {
		unsafe { SpecialPickup_BallFreeze_TA_Get_bMaintainMomentum(self.addr()) }
	}
	fn get_b_touched(&self) -> bool {
		unsafe { SpecialPickup_BallFreeze_TA_Get_bTouched(self.addr()) }
	}
	fn get_time_to_stop(&self) -> f32 {
		unsafe { SpecialPickup_BallFreeze_TA_Get_TimeToStop(self.addr()) }
	}
	fn get_stop_momentum_percentage(&self) -> f32 {
		unsafe { SpecialPickup_BallFreeze_TA_Get_StopMomentumPercentage(self.addr()) }
	}
	fn get_ball(&self) -> Option<BallWrapper> {
		unsafe { BallWrapper::try_new(SpecialPickup_BallFreeze_TA_Get_Ball(self.addr())) }
	}
	...
	fn get_orig_speed(&self) -> f32 {
		unsafe { SpecialPickup_BallFreeze_TA_Get_OrigSpeed(self.addr()) }
	}
	fn get_rep_orig_speed(&self) -> f32 {
		unsafe { SpecialPickup_BallFreeze_TA_Get_RepOrigSpeed(self.addr()) }
	}
	fn get_freeze_fx(&self) -> Option<FXActorWrapper> {
		unsafe { FXActorWrapper::try_new(SpecialPickup_BallFreeze_TA_Get_FreezeFX(self.addr())) }
	}
	fn pickup_end(&self) {
		unsafe {
