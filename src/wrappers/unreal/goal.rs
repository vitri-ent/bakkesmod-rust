...
pub trait Goal: Object {
	fn get_goal_orientation(&self) -> Option<ActorWrapper> {
		unsafe { ActorWrapper::try_new(Goal_TA_Get_GoalOrientation(self.addr())) }
	}
	...
	fn get_team_num(&self) -> u8 {
		unsafe { Goal_TA_Get_TeamNum(self.addr()) }
	}
	fn get_score_fx(&self) -> Option<FXActorWrapper> {
		unsafe { FXActorWrapper::try_new(Goal_TA_Get_ScoreFX(self.addr())) }
	}
	...
	fn get_b_no_goal_indicator(&self) -> bool {
		unsafe { Goal_TA_Get_bNoGoalIndicator(self.addr()) }
	}
	fn get_b_only_goals_from_direction(&self) -> bool {
		unsafe { Goal_TA_Get_bOnlyGoalsFromDirection(self.addr()) }
	}
	fn get_b_show_focus_extent(&self) -> bool {
		unsafe { Goal_TA_Get_bShowFocusExtent(self.addr()) }
	}
	fn get_goal_direction(&self) -> Option<ActorWrapper> {
		unsafe { ActorWrapper::try_new(Goal_TA_Get_GoalDirection(self.addr())) }
	}
	fn get_points_to_award(&self) -> i32 {
		unsafe { Goal_TA_Get_PointsToAward(self.addr()) }
	}
	...
	fn get_max_goal_scorer_attach_radius(&self) -> f32 {
		unsafe { Goal_TA_Get_MaxGoalScorerAttachRadius(self.addr()) }
	}
	...
	fn get_min_attach_goal_to_scorer_dot(&self) -> f32 {
		unsafe { Goal_TA_Get_MinAttachGoalToScorerDot(self.addr()) }
	}
