...
pub trait PrimitiveComponent: Object {
	fn get_rb_channel(&self) -> u8 {
		unsafe { PrimitiveComponent_Get_RBChannel(self.addr()) }
	}
	fn get_rb_dominance_group(&self) -> u8 {
		unsafe { PrimitiveComponent_Get_RBDominanceGroup(self.addr()) }
	}
	fn get_b_only_block_actor_movement(&self) -> bool {
		unsafe { PrimitiveComponent_Get_bOnlyBlockActorMovement(self.addr()) }
	}
	fn get_hidden_game(&self) -> bool {
		unsafe { PrimitiveComponent_Get_HiddenGame(self.addr()) }
	}
	fn get_b_owner_no_see(&self) -> bool {
		unsafe { PrimitiveComponent_Get_bOwnerNoSee(self.addr()) }
	}
	fn get_b_only_owner_see(&self) -> bool {
		unsafe { PrimitiveComponent_Get_bOnlyOwnerSee(self.addr()) }
	}
	fn get_b_ignore_owner_hidden(&self) -> bool {
		unsafe { PrimitiveComponent_Get_bIgnoreOwnerHidden(self.addr()) }
	}
	fn get_b_use_as_occluder(&self) -> bool {
		unsafe { PrimitiveComponent_Get_bUseAsOccluder(self.addr()) }
	}
	fn get_b_allow_approximate_occlusion(&self) -> bool {
		unsafe { PrimitiveComponent_Get_bAllowApproximateOcclusion(self.addr()) }
	}
	fn get_b_first_frame_occlusion(&self) -> bool {
		unsafe { PrimitiveComponent_Get_bFirstFrameOcclusion(self.addr()) }
	}
	fn get_b_ignore_near_plane_intersection(&self) -> bool {
		unsafe { PrimitiveComponent_Get_bIgnoreNearPlaneIntersection(self.addr()) }
	}
	fn get_b_accepts_static_decals(&self) -> bool {
		unsafe { PrimitiveComponent_Get_bAcceptsStaticDecals(self.addr()) }
	}
	fn get_b_accepts_dynamic_decals(&self) -> bool {
		unsafe { PrimitiveComponent_Get_bAcceptsDynamicDecals(self.addr()) }
	}
	fn get_b_is_refreshing_decals(&self) -> bool {
		unsafe { PrimitiveComponent_Get_bIsRefreshingDecals(self.addr()) }
	}
	fn get_cast_shadow(&self) -> bool {
		unsafe { PrimitiveComponent_Get_CastShadow(self.addr()) }
	}
	fn get_b_force_direct_light_map(&self) -> bool {
		unsafe { PrimitiveComponent_Get_bForceDirectLightMap(self.addr()) }
	}
	fn get_b_cast_dynamic_shadow(&self) -> bool {
		unsafe { PrimitiveComponent_Get_bCastDynamicShadow(self.addr()) }
	}
	fn get_b_cast_static_shadow(&self) -> bool {
		unsafe { PrimitiveComponent_Get_bCastStaticShadow(self.addr()) }
	}
	fn get_b_self_shadow_only(&self) -> bool {
		unsafe { PrimitiveComponent_Get_bSelfShadowOnly(self.addr()) }
	}
	fn get_b_no_mod_self_shadow(&self) -> bool {
		unsafe { PrimitiveComponent_Get_bNoModSelfShadow(self.addr()) }
	}
	fn get_b_accepts_dynamic_dominant_light_shadows(&self) -> bool {
		unsafe { PrimitiveComponent_Get_bAcceptsDynamicDominantLightShadows(self.addr()) }
	}
	fn get_b_cast_hidden_shadow(&self) -> bool {
		unsafe { PrimitiveComponent_Get_bCastHiddenShadow(self.addr()) }
	}
	fn get_b_cast_shadow_as_two_sided(&self) -> bool {
		unsafe { PrimitiveComponent_Get_bCastShadowAsTwoSided(self.addr()) }
	}
	fn get_b_accepts_lights(&self) -> bool {
		unsafe { PrimitiveComponent_Get_bAcceptsLights(self.addr()) }
	}
	fn get_b_accepts_dynamic_lights(&self) -> bool {
		unsafe { PrimitiveComponent_Get_bAcceptsDynamicLights(self.addr()) }
	}
	fn get_b_use_one_pass_lighting_on_translucency(&self) -> bool {
		unsafe { PrimitiveComponent_Get_bUseOnePassLightingOnTranslucency(self.addr()) }
	}
	fn get_b_use_precomputed_shadows(&self) -> bool {
		unsafe { PrimitiveComponent_Get_bUsePrecomputedShadows(self.addr()) }
	}
	fn get_b_has_explicit_shadow_parent(&self) -> bool {
		unsafe { PrimitiveComponent_Get_bHasExplicitShadowParent(self.addr()) }
	}
	fn get_collide_actors(&self) -> bool {
		unsafe { PrimitiveComponent_Get_CollideActors(self.addr()) }
	}
	fn get_always_check_collision(&self) -> bool {
		unsafe { PrimitiveComponent_Get_AlwaysCheckCollision(self.addr()) }
	}
	fn get_block_actors(&self) -> bool {
		unsafe { PrimitiveComponent_Get_BlockActors(self.addr()) }
	}
	fn get_block_zero_extent(&self) -> bool {
		unsafe { PrimitiveComponent_Get_BlockZeroExtent(self.addr()) }
	}
	fn get_block_non_zero_extent(&self) -> bool {
		unsafe { PrimitiveComponent_Get_BlockNonZeroExtent(self.addr()) }
	}
	fn get_can_block_camera(&self) -> bool {
		unsafe { PrimitiveComponent_Get_CanBlockCamera(self.addr()) }
	}
	fn get_block_rigid_body(&self) -> bool {
		unsafe { PrimitiveComponent_Get_BlockRigidBody(self.addr()) }
	}
	fn get_b_block_foot_placement(&self) -> bool {
		unsafe { PrimitiveComponent_Get_bBlockFootPlacement(self.addr()) }
	}
	fn get_b_disable_all_rigid_body(&self) -> bool {
		unsafe { PrimitiveComponent_Get_bDisableAllRigidBody(self.addr()) }
	}
	fn get_b_skip_rb_geom_creation(&self) -> bool {
		unsafe { PrimitiveComponent_Get_bSkipRBGeomCreation(self.addr()) }
	}
	fn get_b_notify_rigid_body_collision(&self) -> bool {
		unsafe { PrimitiveComponent_Get_bNotifyRigidBodyCollision(self.addr()) }
	}
	fn get_b_fluid_drain(&self) -> bool {
		unsafe { PrimitiveComponent_Get_bFluidDrain(self.addr()) }
	}
	fn get_b_fluid_two_way(&self) -> bool {
		unsafe { PrimitiveComponent_Get_bFluidTwoWay(self.addr()) }
	}
	fn get_b_ignore_radial_impulse(&self) -> bool {
		unsafe { PrimitiveComponent_Get_bIgnoreRadialImpulse(self.addr()) }
	}
	fn get_b_ignore_radial_force(&self) -> bool {
		unsafe { PrimitiveComponent_Get_bIgnoreRadialForce(self.addr()) }
	}
	fn get_b_ignore_force_field(&self) -> bool {
		unsafe { PrimitiveComponent_Get_bIgnoreForceField(self.addr()) }
	}
	fn get_b_use_compartment(&self) -> bool {
		unsafe { PrimitiveComponent_Get_bUseCompartment(self.addr()) }
	}
	fn get_always_load_on_client(&self) -> bool {
		unsafe { PrimitiveComponent_Get_AlwaysLoadOnClient(self.addr()) }
	}
	fn get_always_load_on_server(&self) -> bool {
		unsafe { PrimitiveComponent_Get_AlwaysLoadOnServer(self.addr()) }
	}
	fn get_b_ignore_hidden_actors_membership(&self) -> bool {
		unsafe { PrimitiveComponent_Get_bIgnoreHiddenActorsMembership(self.addr()) }
	}
	fn get_absolute_translation(&self) -> bool {
		unsafe { PrimitiveComponent_Get_AbsoluteTranslation(self.addr()) }
	}
	fn get_absolute_rotation(&self) -> bool {
		unsafe { PrimitiveComponent_Get_AbsoluteRotation(self.addr()) }
	}
	fn get_absolute_scale(&self) -> bool {
		unsafe { PrimitiveComponent_Get_AbsoluteScale(self.addr()) }
	}
	fn get_visibility_id(&self) -> i32 {
		unsafe { PrimitiveComponent_Get_VisibilityId(self.addr()) }
	}
	...
	fn get_scale(&self) -> f32 {
		unsafe { PrimitiveComponent_Get_Scale(self.addr()) }
	}
	...
	fn get_bounds_scale(&self) -> f32 {
		unsafe { PrimitiveComponent_Get_BoundsScale(self.addr()) }
	}
	fn get_last_render_time(&self) -> f32 {
		unsafe { PrimitiveComponent_Get_LastRenderTime(self.addr()) }
	}
	fn get_script_rigid_body_collision_threshold(&self) -> f32 {
		unsafe { PrimitiveComponent_Get_ScriptRigidBodyCollisionThreshold(self.addr()) }
	}
