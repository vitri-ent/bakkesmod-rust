...
pub trait RumblePickupComponent: CarComponent {
	...
	fn get_b_hud_ignore_use_time(&self) -> bool {
		unsafe { SpecialPickup_TA_Get_bHudIgnoreUseTime(self.addr()) }
	}
	fn get_b_has_activated(&self) -> bool {
		unsafe { SpecialPickup_TA_Get_bHasActivated(self.addr()) }
	}
	fn get_b_is_active(&self) -> bool {
		unsafe { SpecialPickup_TA_Get_bIsActive(self.addr()) }
	}
	fn get_activation_duration(&self) -> f32 {
		unsafe { SpecialPickup_TA_Get_ActivationDuration(self.addr()) }
	}
	fn get_pickup_fx_archetype(&self) -> Option<FXActorWrapper> {
		unsafe { FXActorWrapper::try_new(SpecialPickup_TA_Get_PickupFXArchetype(self.addr())) }
	}
	fn get_pickup_fx(&self) -> Option<FXActorWrapper> {
		unsafe { FXActorWrapper::try_new(SpecialPickup_TA_Get_PickupFX(self.addr())) }
	}
	fn has_activated(&self) -> bool {
		unsafe { SpecialPickup_TA_HasActivated(self.addr()) }
	}
	fn get_client_target(&self) -> Option<RBActorWrapper> {
		unsafe { RBActorWrapper::try_new(SpecialPickup_TA_GetClientTarget(self.addr())) }
	}
	...
	fn get_active_time_percent(&self) -> f32 {
		unsafe { SpecialPickup_TA_GetActiveTimePercent(self.addr()) }
	}
	fn pickup_end(&self) {
		unsafe {
	...
	fn get_boost_component(&self) -> Option<BoostWrapper> {
		unsafe { BoostWrapper::try_new(SpecialPickup_TA_GetBoostComponent(self.addr())) }
	}
	...
	fn try_activate(&self, target_override: RBActorWrapper) -> bool {
		unsafe { SpecialPickup_TA_TryActivate(self.addr(), target_override.addr()) }
	}
	fn on_created(&self) {
		unsafe {
	...
	fn can_pickup(&self, in_car: CarWrapper) -> bool {
		unsafe { SpecialPickup_TA_CanPickup(self.addr(), in_car.addr()) }
	}
