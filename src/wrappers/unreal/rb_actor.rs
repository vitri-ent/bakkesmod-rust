...
pub trait RBActor: Actor {
	fn get_max_linear_speed(&self) -> f32 {
		unsafe { RBActor_TA_Get_MaxLinearSpeed(self.addr()) }
	}
	fn get_max_angular_speed(&self) -> f32 {
		unsafe { RBActor_TA_Get_MaxAngularSpeed(self.addr()) }
	}
	fn get_b_disable_sleeping(&self) -> bool {
		unsafe { RBActor_TA_Get_bDisableSleeping(self.addr()) }
	}
	fn get_b_replay_actor(&self) -> bool {
		unsafe { RBActor_TA_Get_bReplayActor(self.addr()) }
	}
	fn get_b_frozen(&self) -> bool {
		unsafe { RBActor_TA_Get_bFrozen(self.addr()) }
	}
	fn get_b_ignore_syncing(&self) -> bool {
		unsafe { RBActor_TA_Get_bIgnoreSyncing(self.addr()) }
	}
	fn get_b_phys_initialized(&self) -> bool {
		unsafe { RBActor_TA_Get_bPhysInitialized(self.addr()) }
	}
	...
	fn get_sync_error_angle(&self) -> f32 {
		unsafe { RBActor_TA_Get_SyncErrorAngle(self.addr()) }
	}
	...
	fn get_fx_actor_archetype(&self) -> Option<FXActorWrapper> {
		unsafe { FXActorWrapper::try_new(RBActor_TA_Get_FXActorArchetype(self.addr())) }
	}
	fn get_fx_actor(&self) -> Option<FXActorWrapper> {
		unsafe { FXActorWrapper::try_new(RBActor_TA_Get_FXActor(self.addr())) }
	}
	fn get_last_rb_collisions_frame(&self) -> i32 {
		unsafe { RBActor_TA_Get_LastRBCollisionsFrame(self.addr()) }
	}
	fn get_welded_actor(&self) -> Option<RBActorWrapper> {
		unsafe { RBActorWrapper::try_new(RBActor_TA_Get_WeldedActor(self.addr())) }
	}
	fn get_welded_to(&self) -> Option<RBActorWrapper> {
		unsafe { RBActorWrapper::try_new(RBActor_TA_Get_WeldedTo(self.addr())) }
	}
	fn get_pre_weld_mass(&self) -> f32 {
		unsafe { RBActor_TA_Get_PreWeldMass(self.addr()) }
	}
	...
	fn get_physics_frame(&self) -> i32 {
		unsafe { RBActor_TA_GetPhysicsFrame(self.addr()) }
	}
	fn get_physics_time(&self) -> f32 {
		unsafe { RBActor_TA_GetPhysicsTime(self.addr()) }
	}
	fn init_ak(&self) {
		unsafe {
