...
pub trait PerfStatGraph: StatGraph {
	fn get_fps(&self) -> Option<SampleHistoryWrapper> {
		unsafe { SampleHistoryWrapper::try_new(PerfStatGraph_TA_Get_FPS(self.addr())) }
	}
	fn get_frame_time(&self) -> Option<SampleHistoryWrapper> {
		unsafe { SampleHistoryWrapper::try_new(PerfStatGraph_TA_Get_FrameTime(self.addr())) }
	}
	fn get_game_thread_time(&self) -> Option<SampleHistoryWrapper> {
		unsafe { SampleHistoryWrapper::try_new(PerfStatGraph_TA_Get_GameThreadTime(self.addr())) }
	}
	fn get_render_thread_time(&self) -> Option<SampleHistoryWrapper> {
		unsafe { SampleHistoryWrapper::try_new(PerfStatGraph_TA_Get_RenderThreadTime(self.addr())) }
	}
	fn get_gpu_frame_time(&self) -> Option<SampleHistoryWrapper> {
		unsafe { SampleHistoryWrapper::try_new(PerfStatGraph_TA_Get_GPUFrameTime(self.addr())) }
	}
	...
	fn get_max_fps(&self) -> f32 {
		unsafe { PerfStatGraph_TA_Get_MaxFPS(self.addr()) }
	}
	fn get_target_fps(&self) -> f32 {
		unsafe { PerfStatGraph_TA_Get_TargetFPS(self.addr()) }
	}
