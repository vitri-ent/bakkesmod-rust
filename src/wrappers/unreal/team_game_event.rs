...
pub trait TeamGameEvent: GameEvent {
	...
	fn get_max_team_size(&self) -> i32 {
		unsafe { GameEvent_Team_TA_Get_MaxTeamSize(self.addr()) }
	}
	fn get_num_bots(&self) -> i32 {
		unsafe { GameEvent_Team_TA_Get_NumBots(self.addr()) }
	}
	fn get_b_mute_opposite_teams(&self) -> bool {
		unsafe { GameEvent_Team_TA_Get_bMuteOppositeTeams(self.addr()) }
	}
	fn get_b_disable_muting_other_team(&self) -> bool {
		unsafe { GameEvent_Team_TA_Get_bDisableMutingOtherTeam(self.addr()) }
	}
	fn get_b_forfeit(&self) -> bool {
		unsafe { GameEvent_Team_TA_Get_bForfeit(self.addr()) }
	}
	fn get_b_unfair_teams(&self) -> bool {
		unsafe { GameEvent_Team_TA_Get_bUnfairTeams(self.addr()) }
	}
	fn get_b_always_auto_select_team(&self) -> bool {
		unsafe { GameEvent_Team_TA_Get_bAlwaysAutoSelectTeam(self.addr()) }
	}
	...
	fn choose_team(&self, team_index: i32, player: PlayerControllerWrapper) -> bool {
		unsafe { GameEvent_Team_TA_ChooseTeam(self.addr(), team_index, player.addr()) }
	}
	fn can_change_team(&self) -> bool {
		unsafe { GameEvent_Team_TA_CanChangeTeam(self.addr()) }
	}
	...
	fn find_bot_replacement(&self, pri: PriWrapper) -> bool {
		unsafe { GameEvent_Team_TA_FindBotReplacement(self.addr(), pri.addr()) }
	}
	fn end_game(&self) {
		unsafe {
	...
	fn all_teams_have_humans(&self) -> bool {
		unsafe { GameEvent_Team_TA_AllTeamsHaveHumans(self.addr()) }
	}
	fn all_teams_created(&self) -> bool {
		unsafe { GameEvent_Team_TA_AllTeamsCreated(self.addr()) }
	}
