...
pub trait Car: Vehicle {
	...
	fn set_car_rotation(&self, rotation: Rotator) {
		...
		unsafe {
			Car_SetCarRotation(self.addr(), rotation);
		}
	}

	fn force_boost(&self, force: bool) {
		unsafe {
			Car_ForceBoost(self.addr(), force);
		}
	}
	...
	fn get_input(&self) -> VehicleInputs {
		...
		unsafe {
			Car_GetInput(self.addr(), result_ptr);
		}
		result
	}
	...
	fn set_input(&self, input: VehicleInputs) {
		...
		unsafe {
			Car_SetInput(self.addr(), input);
		}
	}

	fn destroy(&self) {
		unsafe {
			Car_Destroy(self.addr());
		}
	}
	...
	fn get_flip_component(&self) -> Option<FlipCarComponentWrapper> {
		unsafe { FlipCarComponentWrapper::try_new(Car_TA_Get_FlipComponent(self.addr())) }
	}
	fn get_demolish_target(&self) -> u8 {
		unsafe { Car_TA_Get_DemolishTarget(self.addr()) }
	}
	fn get_demolish_speed(&self) -> u8 {
		unsafe { Car_TA_Get_DemolishSpeed(self.addr()) }
	}
	fn get_b_loadout_set(&self) -> bool {
		unsafe { Car_TA_Get_bLoadoutSet(self.addr()) }
	}
	fn get_b_demolish_on_opposing_ground(&self) -> bool {
		unsafe { Car_TA_Get_bDemolishOnOpposingGround(self.addr()) }
	}
	fn get_b_was_on_opposing_ground(&self) -> bool {
		unsafe { Car_TA_Get_bWasOnOpposingGround(self.addr()) }
	}
	fn get_b_demolish_on_goal_zone(&self) -> bool {
		unsafe { Car_TA_Get_bDemolishOnGoalZone(self.addr()) }
	}
	fn get_b_was_in_goal_zone(&self) -> bool {
		unsafe { Car_TA_Get_bWasInGoalZone(self.addr()) }
	}
	fn get_b_override_handbrake_on(&self) -> bool {
		unsafe { Car_TA_Get_bOverrideHandbrakeOn(self.addr()) }
	}
	fn get_b_override_boost_on(&self) -> bool {
		unsafe { Car_TA_Get_bOverrideBoostOn(self.addr()) }
	}
	fn get_exit_fx_archetype(&self) -> Option<FXActorWrapper> {
		unsafe { FXActorWrapper::try_new(Car_TA_Get_ExitFXArchetype(self.addr())) }
	}
	fn get_max_time_for_dodge(&self) -> f32 {
		unsafe { Car_TA_Get_MaxTimeForDodge(self.addr()) }
	}
	fn get_last_wheels_hit_ball_time(&self) -> f32 {
		unsafe { Car_TA_Get_LastWheelsHitBallTime(self.addr()) }
	}
	fn get_replicated_car_scale(&self) -> f32 {
		unsafe { Car_TA_Get_ReplicatedCarScale(self.addr()) }
	}
	fn get_body_fx_actor(&self) -> Option<FXActorWrapper> {
		unsafe { FXActorWrapper::try_new(Car_TA_Get_BodyFXActor(self.addr())) }
	}
	fn get_attacker_pri(&self) -> Option<PriWrapper> {
		unsafe { PriWrapper::try_new(Car_TA_Get_AttackerPRI(self.addr())) }
	}
	...
	fn get_attached_pickup(&self) -> Option<RumblePickupComponentWrapper> {
		unsafe { RumblePickupComponentWrapper::try_new(Car_TA_Get_AttachedPickup(self.addr())) }
	}
	...
	fn get_added_ball_force_multiplier(&self) -> f32 {
		unsafe { Car_TA_Get_AddedBallForceMultiplier(self.addr()) }
	}
	fn get_added_car_force_multiplier(&self) -> f32 {
		unsafe { Car_TA_Get_AddedCarForceMultiplier(self.addr()) }
	}
	fn get_game_event(&self) -> Option<GameEventWrapper> {
		unsafe { GameEventWrapper::try_new(Car_TA_Get_GameEvent(self.addr())) }
	}
	fn get_max_drive_backwards_speed(&self) -> f32 {
		unsafe { Car_TA_GetMaxDriveBackwardsSpeed(self.addr()) }
	}
	fn get_max_drive_forward_speed(&self) -> f32 {
		unsafe { Car_TA_GetMaxDriveForwardSpeed(self.addr()) }
	}
	...
	fn can_demolish(&self, hit_car: CarWrapper) -> bool {
		unsafe { Car_TA_CanDemolish(self.addr(), hit_car.addr()) }
	}
	...
	fn is_dodging(&self) -> bool {
		unsafe { Car_TA_IsDodging(self.addr()) }
	}
	...
	fn any_wheel_touching_ground(&self) -> bool {
		unsafe { Car_TA_AnyWheelTouchingGround(self.addr()) }
	}
	fn give_car_component(&self, component_archetype: CarComponentWrapper, activator: PriWrapper) -> Option<CarComponentWrapper> {
		unsafe { CarComponentWrapper::try_new(Car_TA_GiveCarComponent(self.addr(), component_archetype.addr(), activator.addr())) }
	}
	...
	fn update_team_loadout(&self) -> bool {
		unsafe { Car_TA_UpdateTeamLoadout(self.addr()) }
	}
	...
	fn get_loadout_team_index(&self) -> i32 {
		unsafe { Car_TA_GetLoadoutTeamIndex(self.addr()) }
	}
	fn get_preview_team_index(&self) -> i32 {
		unsafe { Car_TA_GetPreviewTeamIndex(self.addr()) }
	}
	fn has_team(&self) -> bool {
		unsafe { Car_TA_HasTeam(self.addr()) }
	}
