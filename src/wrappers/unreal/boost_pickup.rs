...
pub trait BoostPickup: VehiclePickup {
	fn get_boost_amount(&self) -> f32 {
		unsafe { VehiclePickup_Boost_TA_Get_BoostAmount(self.addr()) }
	}
	fn get_boost_type(&self) -> u8 {
		unsafe { VehiclePickup_Boost_TA_Get_BoostType(self.addr()) }
	}
	...
	fn can_pickup(&self, car: CarWrapper) -> bool {
		unsafe { VehiclePickup_Boost_TA_CanPickup(self.addr(), car.addr()) }
	}
}
