...
pub trait TrainingEditorSaveData: Object {
	...
	fn get_type(&self) -> u8 {
		unsafe { TrainingEditorData_TA_Get_Type(self.addr()) }
	}
	fn get_difficulty(&self) -> u8 {
		unsafe { TrainingEditorData_TA_Get_Difficulty(self.addr()) }
	}
	...
	fn get_num_rounds(&self) -> i32 {
		unsafe { TrainingEditorData_TA_Get_NumRounds(self.addr()) }
	}
	fn get_created_at(&self) -> i64 {
		unsafe { TrainingEditorData_TA_Get_CreatedAt(self.addr()) }
	}
	fn get_updated_at(&self) -> i64 {
		unsafe { TrainingEditorData_TA_Get_UpdatedAt(self.addr()) }
	}
