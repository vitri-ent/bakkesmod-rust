...
pub trait FlipCarComponent: CarComponent {
	fn get_flip_car_impulse(&self) -> f32 {
		unsafe { CarComponent_FlipCar_TA_Get_FlipCarImpulse(self.addr()) }
	}
	fn get_flip_car_torque(&self) -> f32 {
		unsafe { CarComponent_FlipCar_TA_Get_FlipCarTorque(self.addr()) }
	}
	fn get_flip_car_time(&self) -> f32 {
		unsafe { CarComponent_FlipCar_TA_Get_FlipCarTime(self.addr()) }
	}
	fn get_b_flip_right(&self) -> bool {
		unsafe { CarComponent_FlipCar_TA_Get_bFlipRight(self.addr()) }
	}
	fn init_flip(&self) {
		unsafe {
	...
	fn can_activate(&self) -> bool {
		unsafe { CarComponent_FlipCar_TA_CanActivate(self.addr()) }
	}
	fn on_created(&self) {
		unsafe {
