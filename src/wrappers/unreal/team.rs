...
pub trait Team: TeamInfo {
	...
	fn get_game_event(&self) -> Option<TeamGameEventWrapper> {
		unsafe { TeamGameEventWrapper::try_new(Team_TA_Get_GameEvent(self.addr())) }
	}
	...
	fn get_club_id(&self) -> i64 {
		unsafe { Team_TA_Get_ClubID(self.addr()) }
	}
	fn get_b_forfeit(&self) -> bool {
		unsafe { Team_TA_Get_bForfeit(self.addr()) }
	}
	...
	fn is_single_party(&self) -> bool {
		unsafe { Team_TA_IsSingleParty(self.addr()) }
	}
	...
	fn get_num_bots(&self) -> i32 {
		unsafe { Team_TA_GetNumBots(self.addr()) }
	}
	fn get_num_humans(&self) -> i32 {
		unsafe { Team_TA_GetNumHumans(self.addr()) }
	}
