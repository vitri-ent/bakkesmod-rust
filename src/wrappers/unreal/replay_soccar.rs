...
pub trait ReplaySoccar: Replay {
	fn get_team_size(&self) -> i32 {
		unsafe { Replay_Soccar_TA_Get_TeamSize(self.addr()) }
	}
	fn get_unfair_team_size(&self) -> i32 {
		unsafe { Replay_Soccar_TA_Get_UnfairTeamSize(self.addr()) }
	}
	fn get_b_unfair_bots(&self) -> bool {
		unsafe { Replay_Soccar_TA_Get_bUnfairBots(self.addr()) }
	}
	fn get_primary_player_team(&self) -> i32 {
		unsafe { Replay_Soccar_TA_Get_PrimaryPlayerTeam(self.addr()) }
	}
	fn get_team0_score(&self) -> i32 {
		unsafe { Replay_Soccar_TA_Get_Team0Score(self.addr()) }
	}
	fn get_team1_score(&self) -> i32 {
		unsafe { Replay_Soccar_TA_Get_Team1Score(self.addr()) }
	}
