...
pub trait TimeBombPickup: RumblePickupComponent {
	fn get_radius(&self) -> f32 {
		unsafe { SpecialPickup_TimeBomb_TA_Get_Radius(self.addr()) }
	}
	fn get_almost_ready_duration(&self) -> f32 {
		unsafe { SpecialPickup_TimeBomb_TA_Get_AlmostReadyDuration(self.addr()) }
	}
	fn get_start_mat_speed(&self) -> f32 {
		unsafe { SpecialPickup_TimeBomb_TA_Get_StartMatSpeed(self.addr()) }
	}
	fn get_almost_ready_mat_speed(&self) -> f32 {
		unsafe { SpecialPickup_TimeBomb_TA_Get_AlmostReadyMatSpeed(self.addr()) }
	}
	fn get_impulse_force(&self) -> f32 {
		unsafe { SpecialPickup_TimeBomb_TA_Get_ImpulseForce(self.addr()) }
	}
	fn get_car_vertical_force(&self) -> f32 {
		unsafe { SpecialPickup_TimeBomb_TA_Get_CarVerticalForce(self.addr()) }
	}
	fn get_car_torque(&self) -> f32 {
		unsafe { SpecialPickup_TimeBomb_TA_Get_CarTorque(self.addr()) }
	}
	fn get_b_demolish(&self) -> bool {
		unsafe { SpecialPickup_TimeBomb_TA_Get_bDemolish(self.addr()) }
	}
	fn get_b_impulse(&self) -> bool {
		unsafe { SpecialPickup_TimeBomb_TA_Get_bImpulse(self.addr()) }
	}
	fn pickup_end(&self) {
		unsafe {
