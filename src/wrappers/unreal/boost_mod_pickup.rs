...
pub trait BoostModPickup: RumblePickupComponent {
	fn get_b_unlimited_boost(&self) -> bool {
		unsafe { SpecialPickup_BoostMod_TA_Get_bUnlimitedBoost(self.addr()) }
	}
	fn get_boost_strength(&self) -> f32 {
		unsafe { SpecialPickup_BoostMod_TA_Get_BoostStrength(self.addr()) }
	}
	fn get_old_boost_strength(&self) -> f32 {
		unsafe { SpecialPickup_BoostMod_TA_Get_OldBoostStrength(self.addr()) }
	}
	fn pickup_end(&self) {
		unsafe {
