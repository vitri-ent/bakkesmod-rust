...
pub trait InputBufferGraph: StatGraph {
	fn get_buffer(&self) -> Option<SampleHistoryWrapper> {
		unsafe { SampleHistoryWrapper::try_new(InputBufferGraph_TA_Get_Buffer(self.addr())) }
	}
	fn get_buffer_target(&self) -> Option<SampleHistoryWrapper> {
		unsafe { SampleHistoryWrapper::try_new(InputBufferGraph_TA_Get_BufferTarget(self.addr())) }
	}
	fn get_over_under_frames(&self) -> Option<SampleHistoryWrapper> {
		unsafe { SampleHistoryWrapper::try_new(InputBufferGraph_TA_Get_OverUnderFrames(self.addr())) }
	}
	fn get_physics_rate(&self) -> Option<SampleHistoryWrapper> {
		unsafe { SampleHistoryWrapper::try_new(InputBufferGraph_TA_Get_PhysicsRate(self.addr())) }
	}
	fn get_max_physics_rate(&self) -> f32 {
		unsafe { InputBufferGraph_TA_Get_MaxPhysicsRate(self.addr()) }
	}
	fn get_min_physics_rate(&self) -> f32 {
		unsafe { InputBufferGraph_TA_Get_MinPhysicsRate(self.addr()) }
	}
